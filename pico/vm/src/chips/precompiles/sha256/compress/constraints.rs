...
use crate::{
    chips::{
        ...
        gadgets::{
            add::AddGadget, add5::Add5Operation, and::AndOperation,
            fixed_rotate_right::FixedRotateRightOperation, not::NotOperation, xor::XorOperation,
        },
    },
    compiler::word::Word,
    ...
    CB::Var: Sized,
{
    fn eval(&self, builder: &mut CB) {
        ...
        let next: &ShaCompressCols<CB::Var> = (*next).borrow();
...
impl<F: PrimeField32> ShaCompressChip<F> {
    ...
    fn eval_memory<CB: ChipBuilder<F>>(&self, builder: &mut CB, local: &ShaCompressCols<CB::Var>) {
        ...
        builder.when(local.is_initialize).assert_eq(
            local.mem_addr,
            local.h_ptr + cycle_step.clone() * CB::Expr::from_canonical_u32(4),
        );
    ...
    fn eval_compression_ops<CB: ChipBuilder<F>>(
        ...
        for i in 0..64 {
            ...
            builder
                ...
                .assert_all_eq(local.k, Word::<CB::F>::from(SHA_COMPRESS_K[i]));
        }
